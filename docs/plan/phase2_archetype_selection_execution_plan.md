...
- `Phase2Evaluator`:
  - 支持 val 快速评估与完整 walk-forward 评估。
...
完善或新增逻辑文件（可并入 config/report/evaluator，不强制独立文件）:
...
pytest tests/unit/evaluation/test_phase2_numerical_safety.py
```
...
- `phase2_report.json` 包含 horizon 覆盖、PPO 健康、train/val/test 指标、risk/behavior warnings、stress summary 与 equity curve summary。
- `best_selector.pt`、`last_selector.pt`、`phase2_horizon_index_*.feather`、`phase2_per_horizon_records_*.feather`、`phase2_checkpoint_manifest.json` 可被后续阶段读取。
- 代码审查清单全部通过，尤其是:
  - test label 不泄漏
