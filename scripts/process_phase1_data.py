...
def build_parser() -> argparse.ArgumentParser:
    ...
    这里集中声明采样、分层、reward 对齐和 DP 并行参数，是为了让
    manifest 能由命令行配置完整复现，而不是依赖脚本里的隐式默认值。
    """
...
def build_data_process_config(args: argparse.Namespace) -> Phase1DataProcessConfig:
    ...
    这个函数负责合并默认配置、显式 override 和本地 smoke 放宽项，确保
    后续处理阶段只依赖 ``Phase1DataProcessConfig``，不再读取 argparse 状态。
    """
...
def assert_prospective_diagnostic(args: argparse.Namespace) -> None:
    ...
    这是启动前的快速失败检查；同类运行时检查也会在 processor 内执行，
    但这里能更早给出面向用户的命令行错误。
    """
...
def main(argv: Sequence[str] | None = None) -> int:
    """命令行入口：解析参数、配置日志并执行 processor。

    返回整型退出码而不是直接退出，方便单测和外部调度器复用该入口。
    """
    parser = build_parser()
