...
## Experiments That Did Not Carry Forward

The logs in `docs/ai_chat_log/` and `docs/future_phase1_change.md` include several useful negative results. Some of these experiments were valuable diagnostics, but they either failed outright, regressed trading quality, or were not strong enough to become the current mainline. Source filenames below refer to `docs/ai_chat_log/` unless the source is `future_phase1_change.md`.

| Area | Attempt | Observed Problem | Current Takeaway | Source |
|---|---|---|---|---|
| Phase I decoder | Raw pointwise MLP decoder without constrained inference | No temporal consistency: frequent action switching, low change-point accuracy, and high execution cost. | Raw decoder argmax should remain diagnostic only; constrained decoding is required for single-trade execution. | `2026-04-03_phase1_decoder_optimization_log.md` |
| Phase I decoder | Plain LSTM decoder | Learned the single-trade shape, but collapsed directionally toward long actions; short recall was nearly zero. | Temporal recurrence alone did not solve direction semantics. | `2026-04-03_phase1_decoder_optimization_log.md` |
| Phase I loss | Inverse-frequency class weights | Overcorrected the LSTM collapse in the opposite direction, producing an almost all-short decoder and worse change-point accuracy. | Static class weighting is a poor fix for this decoder collapse mode. | `2026-04-03_phase1_decoder_optimization_log.md` |
| Phase I loss | Focal loss plus mild class weights | Also biased the decoder toward short actions and did not recover useful long/short semantics. | Loss-only reweighting did not address the root cause. | `2026-04-03_phase1_decoder_optimization_log.md` |
| Phase I decoder | Teacher forcing with previous-action input | Produced exposure bias at inference time; archetypes became almost identical and `z_q` was effectively ignored. | Avoid autoregressive teacher-forcing variants for this setting. | `2026-04-03_phase1_decoder_optimization_log.md` |
| Phase II selector | Early selector training with strong imitation pressure | The selector collapsed toward a small set of archetypes, had near-random agreement with DP labels, and behaved close to best-fixed archetype selection. | Current Phase II uses lower/annealed imitation pressure and additional diagnostics. | `2026-04-03_phase1_decoder_optimization_log.md`, `2026-04-12_phase2_optimization_log.md` |
| Phase II generalization | AL selector over-selecting the flat-like `k=8` archetype | `k=8` was valid on train as a no-trade archetype, but the selector chose it on profitable val horizons because of distribution shift and overconfidence. | Treat this as a selector generalization issue, not as a bad archetype by itself. | `2026-04-12_phase2_optimization_log.md` |
| Phase III refinement | Full AL Phase III run after Phase II | Phase III completed training, but `phase3_eval` matched `phase2_eval` exactly and operation CSV checksums were identical. | Phase III is not expected to fix high-level archetype selection mistakes under the paper-style local refinement role. | `2026-04-12_phase3_optimization_log.md` |
| DP sampling | `90%` stratified plus `10%` importance start sampling | Shifted the Phase I training distribution toward high-return/high-volatility windows and degraded Phase I/II returns. | Avoid importance-heavy sampling as a mainline default; pure stratified sampling was more stable. | `2026-04-19_phase1_dp_structure_optimization_log.md` |
| DP sampling | Pure stratified sampling as the only fix | Recovered part of the regression, but `best_fixed` and `realizable_proxy` stayed too low. | Sampling alone does not solve Phase I return semantics. | `2026-04-19_phase1_dp_structure_optimization_log.md`, `future_phase1_change.md` |
| Phase I checkpointing | Strict profit gate without stronger profit semantics | Correctly blocked weak checkpoints, but the run still had no checkpoint good enough for Phase II. | The gate is a guardrail, not a model-quality improvement by itself. | `2026-04-19_phase1_dp_structure_optimization_log.md` |
| Phase I semantics | Strong usage-profit alignment plus separation | Improved correlation symptoms but flattened the return spectrum; genuinely profitable archetypes disappeared. | Alignment must stay light; it should not dominate the Phase I objective. | `2026-04-19_phase1_dp_structure_optimization_log.md` |
| Phase I semantics | Return regression auxiliary head | Continuous return regression was too weak to shape code-level profit semantics; `best_fixed` remained poor. | Return bucket classification worked better than weak scalar regression. | `2026-04-19_phase1_dp_structure_optimization_log.md` |
| Phase I semantics | Return bucket objective without light alignment | Recovered profitable archetypes, but assignment still had negative usage-profit correlation and failed the gate. | Return buckets need light usage-profit alignment in the current recipe. | `2026-04-19_phase1_dp_structure_optimization_log.md` |
| Codebook capacity | Increasing codebook size `K` from 10 to 16/20 | Later trials did not produce useful gains despite the plausible motivation. | `num_archetypes` remains configurable, but larger `K` is not a priority path. | `future_phase1_change.md` |
| Horizon protocol | Multi-scale horizon training with padding | Tried and found ineffective; it also breaks the fixed `h=72` protocol shared by Phase I, Phase II, and evaluation. | Keep the current single-horizon pipeline unless a separate experiment branch redesigns all dependent stages. | `future_phase1_change.md` |
| Feature sets | `short` / `long` cycle feature groups as defaults | Recorded returns were much weaker than the `middle` feature group. | CLI defaults to `--cycle-feature-sets middle`. | `future_phase1_change.md` |
