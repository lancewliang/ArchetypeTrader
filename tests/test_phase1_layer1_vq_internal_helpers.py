...
def test_vq_internal_metrics_use_raw_timestep_and_turnover_errors() -> None:
    ...
    assert isinstance(computation.extra_payload, Phase1VQInternalPayload)
...
def test_vq_internal_metrics_do_not_infer_codebook_size_from_used_codes() -> None:
    ...
    assert computation.extra_payload["code_distribution"] == ()
    assert computation.extra_payload["active_codes"] == ()
...
def test_vq_internal_payload_exposes_code_usage_and_mapping_compatibility() -> None:
    assignment = _assignment(epoch=3, code_ids=[0, 1], active_codes=(0,))
    payload = Phase1VQInternalPayload(
        code_distribution=(0.5, 0.5),
        active_codes=(0,),
        current_assignment=assignment,
        assignment_churn_by_epoch={3: 0.25},
        codebook_size=2,
        codebook_size_available=True,
        code_distribution_sample_count=2,
    )

    assert payload.code_distribution == (0.5, 0.5)
    assert payload.active_codes == (0,)
    assert payload["current_assignment"] is assignment
    assert payload.get("codebook_size") == 2

    restored = Phase1VQInternalPayload.from_dict(payload.to_dict())

    assert restored.code_distribution == payload.code_distribution
    assert restored.active_codes == payload.active_codes
    assert restored.current_assignment.active_codes == assignment.active_codes
    assert restored.assignment_churn_by_epoch == {3: 0.25}
