...
边界:
    ...
    - selector observation 只使用上一分片完整状态序列和当前分片 t 状态窗口。
"""
...
class Phase2SelectionDataset:
    ...
    ``(previous_t_states, current_t_states)``；
...
class Phase2SelectionDatasetBuilder:
    ...
    def __init__(self, *, tsize: TSize = 1) -> None:
        ...
        输入:
            tsize: selector 可见状态的 t 步长。``previous_t_states`` 使用上一分片
                完整状态序列，``current_t_states`` 使用当前分片前 ``tsize`` 个状态。
    ...
    def build_from_horizon_and_labels(
        ...
        输出:
            ``Phase2SelectionDataset``。其中 ``visible_states`` 是
            ``(previous_t_states, current_t_states)``；``sample_ids`` 是当前分片
        ...
        visible_states = self.build_visible_states(horizon_states)
    ...
    def build_visible_states(self, horizon_states: np.ndarray) -> VisibleStatesDataset:
        ...
        输出:
            ``VisibleStatesDataset``，即 ``(previous_t_states, current_t_states)``。
        ...
        if horizon_states.ndim != 3:
            raise ValueError(
                "horizon_states must have shape [sample, horizon, feature_dim]"
            )
        ...
        return previous_t_states, current_t_states
    ...
    def validate_no_future_leakage(
        ...
        if previous_t_states.shape[0] != horizon_states.shape[0]:
            raise ValueError("previous_t_states must match current horizon sample count")
        if previous_t_states.shape[1:] != horizon_states.shape[1:]:
            raise ValueError(
                "previous_t_states must have shape [sample, horizon, feature_dim]"
            )
        if current_t_states.shape != (
            horizon_states.shape[0],
            self.tsize,
            horizon_states.shape[2],
        ):
            raise ValueError(
                "current_t_states must have shape [sample, tsize, feature_dim]"
            )
        if not np.array_equal(current_t_states, horizon_states[:, : self.tsize, :]):
            raise ValueError("current_t_states must match current horizon t states")
    ...
    def to_tensor_dataset(
        ...
        previous_t_states_np, current_t_states_np = dataset.visible_states
